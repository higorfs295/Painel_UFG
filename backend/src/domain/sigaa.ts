// Parser de códigos de horário SIGAA — portado de frontend/src/lib/sigaa.ts.
// Roda também no servidor: nunca confiar na validação do cliente (RF-11).
...
// Conflito entre dois códigos: interseção de slots ocupados.
export function conflicts(a: string, b: string): SlotKey[] {
  const sa = new Set(parseSIGAA(a).slots);
  return parseSIGAA(b).slots.filter(s => sa.has(s));
}
